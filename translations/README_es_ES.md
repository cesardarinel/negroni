...
**Aviso:** ** Aviso: ** Esta libreria es conocida anteriormente como: `github.com/codegangsta/negroni` Github redirigirá automáticamente las solicitudes
a este repositorio, pero recomendamos actualizar sus referencias para  una mayor claridad.


Negroni es un enfoque idiomático del middleware web en Go(quiere decir que trata de usar la forma pura y propia del lenguaje). Es pequeño,
no intrusivo, y fomenta el uso del paquete `net / http`.


Si te gusta la idea de [Martini](https://github.com/go-martini/martini), Pero entiendes que MArtini tiene mucha magia, entonces Negroni es ideal para ti.
...
Después de instalar Go y configurar 
[GOPATH](http://golang.org/doc/code.html#GOPATH), crea tu primer archivo `.go` 
Lo llamaremos  `server.go`.
...
Luego instala el paquete Negroni (**NOTE**: &gt;= **go 1.1** es requerido):
...
Ahora tendras en con el paquete Go `net/http` un webserver corriendo en `localhost:3000`.

### Packaging

Si tu estas en Debian, `negroni` is also available as [a
